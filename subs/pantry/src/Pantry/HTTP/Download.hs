...
import           Conduit                     -- (runConduit, yield, withSinkFileCautious, withSourceFile)
...
import           Pantry.HTTP                 (Request, Response, HttpException, httpJSON, httpLbs, path, checkResponse, parseUrlThrow, parseRequest, setRequestHeader, getResponseHeaders, requestHeaders, getResponseBody, getResponseStatusCode)
...
download warnHttpRetry req destpath = do
    ...
    verifiedDownload warnHttpRetry downloadReq destpath progressHook
...
redownload :: HasLogFunc env
           => (RetryStatus -> RIO env ())
           -> Request
           -> Path Abs File -- ^ destination
           -> RIO env Bool
redownload warnHttpRetry req0 dest = do
    ...
    recoveringHttp warnHttpRetry drRetryPolicyDefault $ catchingHttpExceptions $ liftIO $
