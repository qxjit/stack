...
module Pantry.HTTP.Download.Verified
  ( verifiedDownload
  , recoveringHttp
  , DownloadRequest(..)
...
import              Pantry.HTTP (Request, HttpException, httpSink, getUri, path, getResponseHeaders, hContentLength, hContentMD5)
import              Path
...
recoveringHttp :: forall env a. (RetryStatus -> RIO env ()) -> RetryPolicy -> RIO env a -> RIO env a
recoveringHttp warnHttpRetry retryPolicy =
...
-- | Copied and extended version of Pantry.HTTP.Download.download.
--
...
verifiedDownload warnHttpRetry DownloadRequest{..} destpath progressSink = do
    let req = drRequest
    whenM' (liftIO getShouldDownload) $ do
        logDebug $ "Downloading " <> display (decodeUtf8With lenientDecode (path req))
        liftIO $ createDirectoryIfMissing True dir
        recoveringHttp warnHttpRetry drRetryPolicy $
